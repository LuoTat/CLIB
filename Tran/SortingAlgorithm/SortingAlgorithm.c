...
void QuickSort_glibc(void* Base, size_t NumOfElements, size_t SizeOfElements, CompareFunction Comparator)
{
    ...
    const size_t Max_Thresh = MAX_THRESH * SizeOfElements;    // Maximum length of each sub-array (4 elements in size)

    if (NumOfElements == 0) return;                           // Return directly for empty arrays
    ...
    if (NumOfElements > MAX_THRESH)
    {
        ...
        while (STACK_NOT_EMPTY)
        {
            ...
            if (Comparator((void*)Mid, (void*)Low) < 0) SWAP(Mid, Low, SizeOfElements);
        A:
            Left_ptr  = Low + SizeOfElements;     // Skip the first element with Left_ptr
            Right_ptr = High - SizeOfElements;    // Skip the last element with Right_ptr
            ...
            if ((size_t)(Right_ptr - Low) <= Max_Thresh)                        // Left side is small
            {
                if ((size_t)(High - Left_ptr) <= Max_Thresh) POP(Low, High);    // Left side is small, right side is small, directly pop
                else Low = Left_ptr;                                            // Left side is small, right side is large, start from the right side
            }    // Left side is large
            else if ((size_t)(High - Left_ptr) <= Max_Thresh) High = Right_ptr;    // Left side is large, right side is small, start from the left side
            else if ((Right_ptr - Low) > (High - Left_ptr))                        // Both sides are large, left side is greater than right side
            {
                PUSH(Low, Right_ptr);                                              // Push the left side onto the stack, sort the right side first
                Low = Left_ptr;
            }
            else
            {
                PUSH(Left_ptr, High);                                              // Push the right side onto the stack, sort the left side first
                High = Right_ptr;
            }
        }
    }
...
#define MIN(x, y) ((x) < (y) ? (x) : (y))
    ...
    // Divide the array into sorted sets of 4 elements each
    // Use insertion sort for best sorting
    char* const End_ptr = &Base_ptr[SizeOfElements * (NumOfElements - 1)];                    // Pointer to the last element of the array
    char*       Tmp_ptr = Base_ptr;                                                           // Pointer to the first element of the array
    char*       Thresh  = MIN(End_ptr, Base_ptr + Max_Thresh);                                // End pointer of the first sub-array
    char*       Run_ptr;
    for (Run_ptr = Tmp_ptr + SizeOfElements; Run_ptr <= Thresh; Run_ptr += SizeOfElements)    // Start from the second element to find the smallest element
    {
        if (Comparator((void*)Run_ptr, (void*)Tmp_ptr) < 0) Tmp_ptr = Run_ptr;                // Find an element smaller than Tmp_ptr, update Tmp_ptr
    }
    if (Tmp_ptr != Base_ptr) SWAP(Tmp_ptr, Base_ptr, SizeOfElements);                         // Place the smallest element of the sub-array at the first position
