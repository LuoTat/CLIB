...
int main()
{
    ...
    clock_t Start, End;
    Start = clock();
    for (int i = 0; i < NodeNum; i++) { LTT_BSTree_Insert_Data(BS_Tree, intArray[i]); }
    End = clock();
    printf("Insert Time: %f ms\n", (double)(End - Start) / CLOCKS_PER_SEC * 1000);

    int NodeNumber = LTT_BiTreeNode_Get_NodeNumber(BS_Tree->BiTree.Root);
    printf("NodeNumber: %d\n", NodeNumber);

    int LeafNum = LTT_BiTreeNode_Get_LeafNumber(BS_Tree->BiTree.Root);
    printf("LeafNumber: %d\n", LeafNum);

    int Depth = LTT_BiTreeNode_Get_Depth(BS_Tree->BiTree.Root);
    printf("Depth: %d\n", Depth);

    BinaryTreeNode* findnode = LTT_BSTreeNode_Search_Data(BS_Tree, intArray[rand() % NodeNum]);
    printf("find %d\n", *(int*)findnode->Data);
